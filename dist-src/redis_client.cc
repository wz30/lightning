...
int redis_thru(std::string id, std::string obj_size) {
#ifdef DEBUG
  std::cout << "inside redis throughput function" << std::endl;
#endif
  int status1 = redis_set(id, obj_size);

  int status2 = redis_get(id);

  int status3 = redis_delete(id);
